...
impl RangeDecoder<'_> {
    ...
    fn try_new(reader: &mut BufReader<'_>) -> Result<RangeDecoder<'_>> {
    ...
    fn decode_symbol(&mut self, symbol_frequencies: &[u32], total: u32) -> u32 {
        ...
        for frequency in symbol_frequencies {
            ...
            if lower_symbol_frequency_threshold <= fs
    ...
    fn normalize(&mut self) -> Result<()> {
        while self.range <= (1 << 23) {
            ...
            self.value = ((self.value << 8)
                + u32::from(255 - (carry_bit << 8) | current_byte >> 1))
                & 0x7FFFFFF;
